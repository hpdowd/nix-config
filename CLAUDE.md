...
- **Out-of-store** (`link "X"` → `mkOutOfStoreSymlink`) — a live symlink into the checkout, so edits take effect with no rebuild, but a fresh clone gets a *symlink and no content*. Still required for: `zed` (its UI rewrites `settings.json`), `htop` (`htoprc` on quit), `ncspot`, `gtk-3.0`/`gtk-4.0`/`nwg-look` (nwg-look writes `settings.ini`), `Kvantum` (kvantummanager), `corectrl`. All seven are theming/TUI state — the fix for each is a native home-manager module (`programs.htop`, `gtk.*`, `qt.*`) that generates the file, not a different symlink.
