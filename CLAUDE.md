...
`tiling` and `hud` must agree with each other *and* with `scheme.nix`, and only
`noctalia` may differ. Build-time modes were rejected — they buy only the half
...
| about to undo something that looks redundant | `docs/adr/` — thirty-four records, each carrying the failure that motivated it |
