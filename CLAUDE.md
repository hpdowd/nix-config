...
- **Store-based** (`source = ../../home/X`) — reproducible. A fresh clone plus a rebuild reproduces it exactly, and `~/.config/X` no longer depends on `~/src/nix-config` existing at all. As of 2026-07-30: `helix`, `kitty`, `foot`, `ghostty`, `zsh/conf.d`, `yazi`, `bottom`, `lazygit`, `glow`, `imv`, and `~/.scripts`.
- **Out-of-store** (`link "X"` → `mkOutOfStoreSymlink`) — a live symlink into the checkout, so edits take effect with no rebuild, but a fresh clone gets a *symlink and no content*. Still required for: `mango` (see the `recursive` warning below), `nvim` (lazy.nvim rewrites the tracked `lazy-lock.json` on `:Lazy sync`), `zed` (its UI rewrites `settings.json`), `htop` (`htoprc` on quit), `ncspot`, `gtk-3.0`/`gtk-4.0`/`nwg-look` (nwg-look writes `settings.ini`), `Kvantum` (kvantummanager), `corectrl`.
