...
| `mango` | No module exists, and the mode scripts genuinely need to `cp` into `config.conf` — hence `recursive = true`. **The waybar layouts are the exception**: `modules/home/waybar.nix` generates the four `config*.jsonc` into `~/.config/mango/waybar/` via `xdg.configFile`, which coexists with the recursive link *only* because those four files were deleted from `home/mango/waybar/`. Two owners for one path is an activation failure, not a merge — the same trap as `walker/config.toml` |
