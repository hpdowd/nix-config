...
- **wlogout** — the session menu (lock/suspend/reboot/shutdown/logout) behind the waybar power icon (`custom/power`). **Its glyph is the NixOS logo, `` (nf-linux-nixos), as of 2026-07-31** — set identically in all four layouts. That codepoint is **not** in every Nerd Font: the bar renders in `3270 Nerd Font` (`style.css`), which does have it. Check before swapping in another glyph — `fc-list ':charset=f313' family` lists the fonts that carry it, and a glyph the font lacks renders as an empty box with nothing in any log. Not to be confused with `custom/power-profile`, a different module. Layout and CSS in `mango/wlogout/`; its five PNGs are **vendored into `mango/wlogout/icons/`** and referenced relatively. See NixOS point 3 below for why.
