...
- **No mason** — language servers are taken from `$PATH`, so they must be declared in `modules/home/packages.nix`. **This is shared with helix**, which also only *configures* servers rather than shipping them. The Arch-installed set did not survive the migration and nobody noticed for a day, because a missing server is skipped in silence: `rust-analyzer` was the only one working. `nil`, `lua-language-server`, `bash-language-server`, `marksman`, `taplo` and `yaml-language-server` were declared on 2026-07-30. **Audited again 2026-08-01: `nvim/lua/plugins/lsp.lua` enables 12 servers and 6 are still missing** — `pyright`, `ruff`, `clangd`, `ts_ls`, `texlab`, `tinymist`. So Python, C/C++, TypeScript, LaTeX and Typst have **no LSP at all** in nvim, silently. Formatters are worse: `stylua` and `shfmt` are referenced and absent, and only `rustfmt` is present. Note `texlab`/`tinymist` matter more than they look — the whole writing stack (`vimtex`, `typst-preview`, knap) is built on them. Check with `for s in pyright ruff clangd typescript-language-server texlab tinymist stylua shfmt; do command -v $s >/dev/null || echo "$s MISSING"; done`. **`hx --health` is the fastest audit** — one line per language, `✘` against any server it cannot find. LSP uses Neovim 0.11+ native `vim.lsp.enable`; completion is `blink.cmp`
...
**Migration discrepancy audit, 2026-08-01.** Re-checked against the live system rather than against this file, which had been wrong three times that week. Most of it is genuinely finished: no `*.hm-bak`, no `~/arch-residue-backup-*`, no `~/src/arch-config`, and exactly **one** out-of-store dotfile entry (`corectrl`, deliberate). What is actually left:

| Gap | State |
|---|---|
| **6 of 12 nvim language servers missing** | `pyright`, `ruff`, `clangd`, `ts_ls`, `texlab`, `tinymist` — Python, C/C++, TypeScript, LaTeX, Typst have no LSP, **silently**. Formatters `stylua`/`shfmt` absent too; only `rustfmt` present. See the Editor section |
| **`~/.config/zen` — 929 MB, in no repo** | 13 extensions, saved logins, history. Survives only via `@home`; a disk failure loses it. Grown ~70 MB since the migration notes |
...
- **Store-based** (`source = ../../home/X`) — reproducible. A fresh clone plus a rebuild reproduces it exactly, and `~/.config/X` no longer depends on `~/src/nix-config` existing at all. As of 2026-07-31: `mango`, `nvim`, `helix`, `kitty`, `foot`, `ghostty`, `zsh/conf.d`, `yazi`, `bottom`, `lazygit`, `glow`, `imv`, `wlogout`, `swaync`, and `~/.scripts`.
...
The same argument applies to `walker/`, `fsel/` and `elephant/`, which have **not** been moved — verify how each actually resolves its config first. What has *not* got a case for moving is `waybar/`, `scripts/`, `tiling/`, `hud/` and `universal/`: those are the compositor and its mode machinery, and waybar needs an explicit `-c` wherever it lives, because the mode scripts switch between four layouts and generate a position-rewritten copy in `~/.local/state/mango/`. Move components one at a time and test each — the failure mode is an app quietly not finding its config and falling back to defaults that look nearly right.
