...
- **Store-based** (`source = ../../home/X`) — reproducible. A fresh clone plus a rebuild reproduces it exactly, and `~/.config/X` no longer depends on `~/src/nix-config` existing at all. As of 2026-07-30: `mango`, `helix`, `kitty`, `foot`, `ghostty`, `zsh/conf.d`, `yazi`, `bottom`, `lazygit`, `glow`, `imv`, and `~/.scripts`.
- **Out-of-store** (`link "X"` → `mkOutOfStoreSymlink`) — a live symlink into the checkout, so edits take effect with no rebuild, but a fresh clone gets a *symlink and no content*. Still required for: `nvim` (lazy.nvim rewrites the tracked `lazy-lock.json` on `:Lazy sync`), `zed` (its UI rewrites `settings.json`), `htop` (`htoprc` on quit), `ncspot`, `gtk-3.0`/`gtk-4.0`/`nwg-look` (nwg-look writes `settings.ini`), `Kvantum` (kvantummanager), `corectrl`.

**`recursive = true` is the important nuance**, and mango is why. A plain `source` makes `~/.config/mango` *one* symlink to a read-only store directory, so nothing can be created inside it. `recursive = true` symlinks each file individually and leaves the **directory** writable — so the mode scripts can still `cp tiling/tiling.conf config.conf`. It does **not** make tracked files writable; those are read-only symlinks either way. That distinction is exactly why mango could convert and `htop` could not.
