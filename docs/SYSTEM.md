...
>
> Until 2026-08-03 it instead rewrote `position` and the margins into a copy at
> `~/.local/state/mango/waybar-config.jsonc` with `sed -E`. That inverted the
> layering — regex mutating typed output Nix had just generated — and needed a
> `margin-swap` placeholder, because sed applies each `-e` to the same line in
> order and a plain top→bottom + bottom→top pair renamed the key and renamed it
> straight back. Enumerating the matrix in `waybar.nix` removed all of it.
>
> The margin mirroring itself still matters: the hud layout uses
...
> **`style.css` was deleted on 2026-08-03 — it was unreachable.** `current-mode`
> only ever holds `tiling` or `hud`, so the non-hud branch of
> `waybar-restart.sh` implied `tiling` and the `*) style.css` arm below it could
> never be taken. 358 lines of stylesheet that nothing had loaded since the
> `dms` mode was removed, while the docs described it as the default. The same
> pass removed `walker/configs/default.toml` and `walker/themes/mango/`, dead
> for the same reason and carrying an off-palette colour scheme. Third instance
> of this pattern, after `active-theme.*` and the `gtk-*-tiling` variants.
